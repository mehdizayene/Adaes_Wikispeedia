...
st.set_page_config(page_title="AdaEs Wikispeedia App", page_icon="📰")#, layout="wide")
...
st.markdown('<p class="big-font"><b>Wikispeedia</b> is a game based on Wikipedia, where you are given a start and end article and they must\
    reach their goal in the fastest time possible only by using Wikipedia hyperlinks.\
    This study aims at analyzing players strategies in order to leverage out insightful conclusions about their navigational behavior.</p>', unsafe_allow_html=True)
...
with col2:
    st.markdown('<p class="big-font"><b>Dataset Metrics :</b></p>', unsafe_allow_html=True)
...
st.markdown('<p class="big-font"><b>Do you think you can do better ?</b></p>', unsafe_allow_html=True)
st.markdown('<p class="big-font"><b>Try out the game if you are confident enough 😜</b></p>', unsafe_allow_html=True)
...
with body:
    ...
    with col2:
        start_button = st.button("Start the game!", on_click=reset_counter)
...
def sisable_submit():
    print('I should desable submit_button here')
def play_round(G_articles, current_node,target_node):
    ...
    return(st.session_state.get('current_node', None),st.session_state.get('target_node', None))          
...
def run():
    ...
    with body:
        ...
        if "current_node" in st.session_state:
            (source,target) = play_round(G_articles, st.session_state["current_node"],st.session_state["target_node"])
            if source==target:
                diff_nodes = st.session_state["num_steps"]-4
                fail_str = "Could do better"
                win_str = "Impressive !"
                time_spent = round(time.time()-st.session_state["start_date"],1)
                time_ranking = get_percent(players_time,time_spent)
                time_well = True if time_ranking < 50 else False
                st.markdown('<p class="big-font"><b>Yeaaaah well done ! 🏆</b></p>', unsafe_allow_html=True)
                st.markdown('<p class="big-font"><b>Here are some statistics about your game 📊: </b></p>', unsafe_allow_html=True)
                col0, col1, col2, col3 = st.columns([0.7,1,1,1])
                col1.metric("Number of hops", st.session_state["num_steps"])
                col2.metric(u'\xa0 Time ', str(time_spent)+" s", "Top "+str(time_ranking)+" %",delta_color="normal" if time_well else "inverse")
                col3.metric("Difference with shortest path",u' \xa0 \xa0 \xa0 \xa0 \xa0'+str(diff_nodes),
                u' \xa0 \xa0 \xa0 \xa0 \xa0'+(fail_str if diff_nodes!= 0 else win_str),
